      ...
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          fetch("/api/v1/weather", options)
            ...
            .then((results) => {
              document.getElementById("locationPermission").style.display =
                "none";
              document.getElementById("weather").innerHTML =
                results.weatherData.weather;
              document.getElementById("weatherIcon").src =
                results.weatherData.iconUrl;
              document.getElementById("temperature").innerText =
                results.weatherData.temperature;
              document.getElementById("weatherCity").innerText =
                results.weatherData.city;
            })
            .catch((err) => {
              console.log(err);
            });
