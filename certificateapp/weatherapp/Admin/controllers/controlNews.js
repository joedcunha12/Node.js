...
exports.addNews = (req, res, next) => {
  News.create(
    ...
    (err, newsItem) => {
      ...
      res
        .status(201)
        .send({ message: "News Item Created successfully", data: newsItem });
    }
  );
};
...
exports.updateNews = (req, res, next) => {
  if (req.params.newsId.match(/^[0-9a-fA-F]{24}$/)) {
    try {
      News.updateOne({ _id: req.params.newsId }, { $set: req.body }).then(
        (result) => {
          if (result.n > 0) {
      ...
      res.status(500).send({
        message: "Changes could not be saved due to a server error.",
        serverError: err,
      });
    }
  } else {
